 ...
 * @since JDK1.6
 */
public class UnstarItemAction implements Runnable {

	private ConfirmActionDialog dialog;
	private MainFrame frMain;
	
	public UnstarItemAction(ConfirmActionDialog dialog,
			MainFrame frMain) {
		this.dialog = dialog;
		this.frMain = frMain;
	}
			...
			for(MyDocumentListEntry entry : SystemRuntime.SelectedItem) {
				frMain.getProcessMessageLabel().setText(
						LanguageResource.getStringValue(
								"dialog.unstar.unstar_process").replace("{1}",
								entry.getEntry().getTitle().getPlainText()));
				IGoSyncDocsBiz.unStarItem(entry);
			}//end of for
